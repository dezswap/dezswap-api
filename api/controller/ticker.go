...
type tickerController struct {
	service.Getter[service.Ticker]
	logger logging.Logger
	tickerMapper
}

func InitTickerController(s service.Getter[service.Ticker], route *gin.RouterGroup, logger logging.Logger) TickerController {
...
func (c *tickerController) register(route *gin.RouterGroup) {
	r := route.Group("/coingecko")
	{
		r.GET("/tickers", c.Tickers)
		r.GET("/tickers/:id", c.Ticker)
	}
